...
function fromPrisma(err: Prisma.PrismaClientKnownRequestError): AppError {
  switch (err.code) {
    case 'P2002':
      return AppError.conflict('A record with these unique fields already exists');
    case 'P2025':
      return AppError.notFound('Record not found');
    case 'P2003':
      return AppError.badRequest('Referenced record does not exist (foreign key)');
    default:
...
export function errorHandler(err: unknown, req: Request, res: Response, _next: NextFunction): void {
  ...
  else if (err instanceof Prisma.PrismaClientKnownRequestError) appErr = fromPrisma(err);
  else if (err instanceof Prisma.PrismaClientValidationError) appErr = AppError.badRequest('Invalid query');
  ...
  const body: {
    error: { code: ApiErrorCode; message: string; traceId: string; details?: unknown; debug?: string };
  } = {
    error: {
      code: appErr.code,
