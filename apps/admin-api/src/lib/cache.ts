...
const jitter = (ttl: number): number =>
  ttl + Math.floor(Math.random() * Math.max(1, ttl * 0.2));
...
const release = (key: string): Promise<unknown> =>
  redis.del(lockKey(key)).catch(() => 0);
...
async function write<T>(
  ...
  await redis
    .set(key, JSON.stringify(box), "EX", hardTtl)
    .catch(() => undefined);
}
...
export async function cached<T>(
  ...
  if (raw) {
    ...
    if (Date.now() < box.soft) return box.v; // fresh

    // Soft-expired: serve stale now, let exactly one caller refresh in the background.
    ...
    return box.v; // stale-while-revalidate
  }

  // Hard miss: single-flight the recompute.
  if (await acquire(key)) {
    try {
  ...
  // Lost the race: wait briefly for the winner to populate
